...
def agent_init():
    ...
    w = np.zeros(1001) # 1-1000 = 0 ,0 not used

    v = np.zeros(1001) # 1-1000 = 0, 0 not used
...
def agent_step(reward, position): # returns NumPy array, reward: floating point, this_observation: NumPy array
    ...
    # do update
    temp_w = np.zeros(1001)

    temp_w[oldS] = 1

    w = w + alpha *(reward + 1 * w[newS] - w[oldS]) * temp_w  # tabular gamma = 1 here in the problem
...
def agent_end(reward):
    ...
    w = w + alpha * (reward - w[oldS]) * temp_w  # tabula gamma = 0.9 use data from last setting
