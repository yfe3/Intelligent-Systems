...
def agent_message(in_message):  # returns string, in_message: string
    ...
    if (in_message == 'ValueFunction'):
        ...
        steps = 50
        true_value=np.zeros((50,50)) #50*50 steps
        for i in range(steps):
            for j in range(steps):
                values=[] #empty
                for A in [0,1,2]:
                    feature_vector = tiles3.tiles(iht, tilings, [8*(-1.2 + (i*1.7 / steps))/(0.5 + 1.2), 8*(-0.07+(j*0.14/steps))/((0.07 + 0.07))], [A]) # formula on assginment
                    true_feature_vector = np.zeros(4096)
                    for k in feature_vector:
                        true_feature_vector[k] = 1
                    values.append(-np.dot(true_feature_vector, w))  # get q
                height = max(values)
                true_value[i][j]=height
        np.save('value',true_value) #save in file
